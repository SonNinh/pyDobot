...
# def detect_color(color_center, img_img, p_blue, p_green, p_red, p_yellow):
#     '''
#     '''
#     fig = pyplot.figure()
#     ax = Axes3D(fig)
...
#     color_mean = img_img.mean(axis=0).mean(axis=0).astype(int)
...
#     img = cv2.cvtColor(red, cv2.COLOR_BGR2HSV)
#     p_red = img.mean(axis=0).mean(axis=0).astype(int)
#     green = cv2.imread("photos/green.png")
#     img = cv2.cvtColor(green, cv2.COLOR_BGR2HSV)
#     p_green = img.mean(axis=0).mean(axis=0).astype(int)
#     blue = cv2.imread("photos/blue.png")
#     img = cv2.cvtColor(blue, cv2.COLOR_BGR2HSV)
#     p_blue = img.mean(axis=0).mean(axis=0).astype(int)
#     yellow = cv2.imread("photos/yellow.png")
#     img = cv2.cvtColor(yellow, cv2.COLOR_BGR2HSV)
#     p_yellow = img.mean(axis=0).mean(axis=0).astype(int)
